          ...
          <div v-for="(folder, key, index) in config.about.sections[currentSection]?.info" :key="key" class="grid grid-cols-2 items-center my-2 font-fira_regular text-menu-text">
            ...
            <transition name="expand">
              <div v-if="folder.files !== undefined && isOpen(folder.title)" class="col-span-2">
                <div v-for="(file, key) in folder.files" :key="key" class="hover:text-white hover:cursor-pointer flex my-2" @click.stop="focusCurrentFile(key)">
      ...
      <div id="left" class="w-full flex flex-col border-right">
        ...
        <div id="commented-text" class="flex h-full w-full lg:border-right overflow-hidden">
          <div class="w-full h-full ml-5 mr-10 lg:my-5 overflow-scroll">
              <CommentedText :text="getCurrentText()" />
          </div>
...
export default {
  ...
  computed: {
    ...
    isOpen() {
      return folderTitle => !!this.openFolders[folderTitle];
    },
  },
  methods: {
    focusCurrentSection(section) {
      ...
      this.subFolder = null
      // Tutup semua folder saat ganti section
      this.openFolders = {};
    ...
    focusCurrentFolder(folder) {
      ...
      // Toggle expand/collapse (tanpa this.$set, gunakan reactive assignment)
      this.openFolders = {
        ...this.openFolders,
        [folder.title]: !this.openFolders[folder.title]
      };
    },
    getCurrentText() {
      const section = this.config.about.sections[this.currentSection];
      if (!section) return '';
      const folderObj = section.info[this.folder];
      if (!folderObj) return '';
      if (this.subFolder && folderObj.files && folderObj.files[this.subFolder]) {
        return folderObj.files[this.subFolder];
      }
      return folderObj.description || '';
    },
    ...
    isOpen(folderTitle) {
      return !!this.openFolders[folderTitle];
    },
  },
  mounted(){
    this.loading = false
